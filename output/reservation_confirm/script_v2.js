/* ===================================================================
   PLK 실시간 예약 페이지 - script_v2.js
   v2: 위임 + 오픈카드 시 결제자 선택 (내가 결제 / 위임자가 결제) 추가
   =================================================================== */
...
var g_payer_delegate = '';           // self | delegate
...
function selectPayment(th) {
  ...
  if (g_visit_type === 'direct') {
    ...
    // 위임: 결제 방법은 결제자 선택에 의해 제어되므로 추가 처리 없음
    $('#payer_block').hide();
    $('#companion_section').hide();
  }
}
